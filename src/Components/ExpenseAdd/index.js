...
function ExpenseAdd() {
  ...
  const copyValue = () => {
    setExpense({ ...expense, expensed: expense.value, familyCost: expense.value, friendsDebit:expense.value });
  };
  ...
  return (
    <div>
      ...
      <p>
        ...
        <span>{expense && expense.value}</span>
      </p>
      <p>
        <span>Date: </span>
        <span>{expense && expense.date}</span>
      </p>

      <p>
        Name:<input type="edit" name="name" value={(expense && expense.name) || ""} onChange={handleChange}></input>
      </p>
      <p>
        Value:<input type="edit" name="value" value={(expense && expense.value) || ""} onChange={handleChange}></input>
        <button onClick={copyValue}>Copy</button>
      </p>
      <p>
        Expensed: :
        <input type="edit" name="expensed" value={(expense && expense.expensed) || ""} onChange={handleChange}></input>
        <button
          onClick={() => {
            setExpense({ ...expense, expensed: 0 });
          }}
        >
          Clear
        </button>
      ...
      <p>
        ...
        <input
          type="edit"
          name="familyCost"
          value={(expense && expense.familyCost) || ""}
          onChange={handleChange}
        ></input>
        <button
          onClick={() => {
            setExpense({ ...expense, familyCost: 0 });
          }}
        >
          Clear
        </button>
      ...
      <p>
        ...
        <input
          type="edit"
          name="friendsDebit"
          value={(expense && expense.friendsDebit) || ""}
          onChange={handleChange}
        ></input>
        <button
          onClick={() => {
            setExpense({ ...expense, friendsDebit: 0 });
          }}
        >
          Clear
        </button>
      ...
      <p>{cache && cache.currencies && cache.currencies.length > 0 && cache.currencies[0].name}</p>
      {
        <p>
          ...
          {cache &&
            cache.currencies &&
            cache.currencies.map((x) => {
              return (
                <span key={x.currencyId}>
                  <input
                    type="radio"
                    value={x.name}
                    onChange={() => changeCurrency(x.currencyId)}
                    checked={expense.currencyId == x.currencyId}
                    name="currency"
                  ></input>
                  {x.name}
                </span>
      ...
      <p>
        ...
        {cache &&
          cache.categories &&
          cache.categories.map((x) => {
            return (
              <span key={x.categoryId}>
                <input
                  type="radio"
                  value={x.name}
                  onChange={() => changeCategory(x.categoryId)}
                  checked={expense.categoryId == x.categoryId}
                  name="category"
                ></input>
                {x.name}
              </span>
